...
class Prefs:
    ...
    def __init__(self, profile):
        ...
        if 'colors' in self.cf.sections():
            self.colors['background'] = gtk.gdk.color_parse(
                self.cf.get('colors', 'background'))
            self.colors['normal'] = gtk.gdk.color_parse(
                self.cf.get('colors', 'normal'))
            self.colors['current'] = gtk.gdk.color_parse(
                self.cf.get('colors', 'current'))
            self.colors['message'] = gtk.gdk.color_parse(
                self.cf.get('colors', 'message'))
    ...
    def save(self):
        ...
        self.cf.set('colors', 'background',
                    self.colors['background'].to_string())
